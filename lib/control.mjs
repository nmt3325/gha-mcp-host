 ...
 * report uncertainty rather than suggest a blind retry. The old system reported this as
 * `state: unknown, output_bytes: 0` and left the caller guessing.
 */
export function listRunning() {
	const out = []
	for (const id of listJobIds()) {
		const metaText = readTextOr(path.join(jobDir(id), "meta.json"))
		if (metaText) {
			let meta
			try { meta = JSON.parse(metaText) } catch { continue }
			if (meta.kind === "file" && meta.state === "running") {
				let alive = false
				if (Number.isSafeInteger(meta.file_worker_pid) && meta.file_worker_pid > 0) {
					try { process.kill(meta.file_worker_pid, 0); alive = true } catch {}
				}
				out.push({ command_id: id, state: alive ? "running" : "lost", alive,
					spawn_gap: false, started_at: meta.started_at ?? null })
			}
			continue
		}
		const pid = jobPid(id)
